...
router.get('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (startDate || endDate) {
            ...
            if (startDate)
                wherePedidos.createdAt.gte = new Date(startDate);
            if (endDate) {
        ...
        if (!platform || platform === 'all' || platform === 'google') {
            promises.push(prisma.googleAdsData.findMany({
        ...
        if (!platform || platform === 'all' || platform === 'facebook') {
            promises.push(prisma.facebookAdsData.findMany({
                where: { ...where, ...whereDate },
                orderBy: { date: 'desc' }
            }));
        }
        ...
        promises.push(prisma.pedido.findMany({
            where: wherePedidos,
            include: { itens: true },
        ...
        const googleMetrics = googleAdsData.reduce((acc, record) => ({
        ...
        const facebookMetrics = facebookAdsData.reduce((acc, record) => ({
        ...
        const pedidosMetrics = pedidos.reduce((acc, pedido) => {
...
function prepareEvolutionData(googleData, facebookData) {
