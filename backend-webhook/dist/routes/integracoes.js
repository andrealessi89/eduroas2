...
async function fetchMetaAdAccounts(accessToken) {
    try {
        const response = await axios_1.default.get(`https://graph.facebook.com/v23.0/me?fields=adaccounts{business_name,account_status,balance,currency,spend_cap,name}&access_token=${accessToken}`);
        if (response.data && response.data.adaccounts && response.data.adaccounts.data) {
            return response.data.adaccounts.data.map((account) => ({
                id: account.id,
                name: account.name,
        ...
        const integracoesSafe = integracoes.map(int => {
            if (int.tipo === 'magazord') {
                ...
                return {
                    ...int,
                    config: {
                        accessToken: int.config.accessToken ?
                            int.config.accessToken.substring(0, 10) + '...' : '',
                        adAccountId: int.config.adAccountId || '',
                        hasToken: !!int.config.accessToken
                    }
                };
            }
            return int;
...
router.post('/magazord', auth_1.authenticateToken, async (req, res) => {
    try {
...
router.get('/meta', auth_1.authenticateToken, async (req, res) => {
    try {
        const integracoes = await prisma.integracao.findMany({
            where: {
                userId: req.user.id,
        ...
        const integracoesSafe = integracoes.map(int => ({
            ...int,
            config: {
                name: int.config.name || int.nome,
                accessToken: int.config.accessToken ?
                    int.config.accessToken.substring(0, 10) + '...' : '',
                hasToken: !!int.config.accessToken,
                adAccounts: int.config.adAccounts || [],
                selectedAccounts: int.config.selectedAccounts || []
            }
        }));
        return res.json({
            success: true,
...
router.post('/meta', auth_1.authenticateToken, async (req, res) => {
    try {
...
router.put('/meta/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const { name, accessToken } = req.body;
        if (!name || !accessToken) {
...
router.patch('/meta/:id/accounts', auth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
...
router.delete('/meta/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
