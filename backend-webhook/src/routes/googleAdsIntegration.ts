...
// Interface para o webhook do Google Ads
...
router.post('/webhook', async (req, res): Promise<any> => {
  try {
    ...
    const data: GoogleAdsWebhookData = req.body;
    ...
    try {
      ...
      await FacebookAdsService.fetchAndSaveInsights(integracao.userId, data.date);
...
router.get('/webhook/test', async (req, res): Promise<any> => {
  return res.json({
    success: true,
